...
export const TURKISH_CITIES: City[] = [
  ...
  { name: "Burdur", lat: 37.7236, lng: 30.2894 },
  ...
  { name: "Muş", lat: 38.7462, lng: 41.5061 },
  ...
  { name: "Tunceli", lat: 39.1079, lng: 39.5479 },
  ...
  { name: "Osmaniye", lat: 37.0742, lng: 36.2474 },
  { name: "Düzce", lat: 40.8438, lng: 31.1565 },
];

export const CITY_NAMES = TURKISH_CITIES.map(c => c.name).sort((a, b) => a.localeCompare(b, 'tr'));

export function getCityCoords(name: string): { lat: number; lng: number } | null {
  const city = TURKISH_CITIES.find(c => c.name.toLowerCase() === name.toLowerCase());
